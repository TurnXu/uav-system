...
namespace stereo_vo {
    ...
    stereo_vo(ros::NodeHandle nh_) : sync_(MySyncPolicy(100), sub_img0_, sub_img1_) {
        ...
        vo_confidence_pub_ = nh_.advertise<std_msgs::Float32>("vo_confidence", 100);
        frame_quality_text_pub_ = nh_.advertise<std_msgs::String>("frame_quality_text", 100);

        if (USE_IMU) {
            imu_sub_ = nh_.subscribe(IMU_TOPIC, 2000, &stereo_vo::imuCallback, this);
    ...
    ros::Publisher vo_confidence_pub_, frame_quality_text_pub_;
    nav_msgs::Path path_;
